...
async  postaddorder(req, res) {
    ...
    for (const slot of slots) {
      ...
      for (const product of products) {
        ...
        if (!productId || !quantity || quantity <= 0) {
          return res.status(400).json({ message: `Invalid product details for "${productName}".` });
        }
  ...
  async refurbishment(req, res) {
    try {
      ...
      const updatedData = {
        productrefurbishment: productrefurbishment || existingRefurbishment.productrefurbishment,
        damagerefurbishment: damagerefurbishment || existingRefurbishment.damagerefurbishment,
        shippingaddressrefurbishment:
          shippingaddressrefurbishment || existingRefurbishment.shippingaddressrefurbishment,
